...
import { env } from "../env";
import { createWebsocketStream, sendEmail } from "./lib";
...
export async function editProject(
    ...
    try {
        ...
        userRole?.project.roles.forEach((role) => {
            stream.send(
                String(role.userId),
                "project-edited",
                JSON.stringify({
...
export async function changeMemberRole(
    ...
    try {
        ...
        await db
            ...
            .where(
                and(eq(roles.userId, userId), eq(roles.projectId, projectId))
            );
...
export async function removeMemberFromProject(
    ...
    try {
        ...
        await db
            .delete(roles)
            .where(
                and(eq(roles.userId, memberId), eq(roles.projectId, projectId))
            );
    ...
    try {
        ...
        const { createdAt, id } = (
            await db
                .insert(comments)
                .values(comment)
                .returning({ createdAt: comments.createdAt, id: comments.id })
        )[0];
